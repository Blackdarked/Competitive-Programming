...
vi bracketsMatching(string& brackets) {
    sti bracketsStack;
    int n = brackets.length();
    vi bracketsPairs(n);

    rep(i, 0, n) {
        if (brackets[i] == '(') bracketsStack.push(i);
        else {
            if (bracketsStack.empty()) bracketsPairs[i] = -1;
            else {
                bracketsPairs[bracketsStack.top()] = i;
                bracketsPairs[i] = bracketsStack.top();
                bracketsStack.pop();
            }
        }
    }
    if (!bracketsStack.empty()) {
        ...
        while(!bracketsStack.empty()) {
            bracketsPairs[bracketsStack.top()] = -1;
            bracketsStack.pop();
        }
    }
