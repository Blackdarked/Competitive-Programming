...
void solve() {
    cin >> n; vll v(n); TRAV(v) { cin >> e; } SORT(v); cin >> x;
    auto binarySearch[](const int& l, const int& r, const int& x) {
        int mid = (l + r) >> 1;
        if (x > mid) binary_search(mid + 1, r, x);
        else if (x < mid) binary_search(l, mid, x);
        else cout << mid << ' ' << mid + 1;
    };
    binarySearch(0, n, x);
}
