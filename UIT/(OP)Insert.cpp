...
int Insert(Hashtable& ht, Hocsinh x) {
    ...
    int i = x.Maso % ht.M, j = 1, cprob = 0;

    while (ht.table != NULL and ht.table[i].Maso != EMPTY and cprob < ((ht.n + 1) >> 1)) {
        i = (x.Maso % ht.M + j) % ht.M;
        j++;
    }

    if (ht.table == NULL or ht.table[i].Maso == EMPTY) {
        ht.table[i] = x;
        ht.n++;
        return 1;
    }
