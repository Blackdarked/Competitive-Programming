...
int Rand(int l, int r) { return l + rd() % (r - l); }
...
ll partition(vector<Point>& v, ll l, ll h) {
    ...
    ll i = l - 1;
    rep(j, l, h) {
        if ((v[j].x < pivot.x) or (v[j].x == pivot.x and v[j].y > pivot.y) or (v[j].x == pivot.x and v[j].y == pivot.y and v[j].z < pivot.z)) {
            i++;
            swap(v[i], v[j]);
        }
    }
